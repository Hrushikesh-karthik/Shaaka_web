...
def view_cart():
    ...
    crops_total = sum(item['price'] for item in crops_cart)
...
def update_crop_quantity():
    ...
    if action == 'increase':
        cursor.execute('UPDATE cart SET quantity = quantity + 1 WHERE id = %s AND customer_id = %s', (cart_id, customer_id))
    elif action == 'decrease':
        cursor.execute('UPDATE cart SET quantity = GREATEST(quantity - 1, 1) WHERE id = %s AND customer_id = %s', (cart_id, customer_id))
